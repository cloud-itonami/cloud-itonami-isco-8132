(ns photolab.advisor
  "Photographic Products Machine Operations Coordination Advisor —
  proposing a plant scheduling/logistics coordination operation (log a
  work record, schedule a crew operation, flag a safety concern,
  coordinate a darkroom-chemical/film-stock supply order) from a crew
  roster, plant registration and safety-reporting policy. Swappable
  mock/llm; the advisor ONLY proposes — `photolab.governor`
  ...
  proposes to directly finalize a processing-operation-execution
  decision or a chemical-safety-clearance decision, and never proposes
  to override a plant safety officer's judgment — those stay
  permanently out of this actor's scope. Modeled on
  cloud-itonami-isco-8112's mineralplant.advisor (closest domain
  shape).
...
(defn- rationale-for [op operator-id plant-id hazard-type]
  (case op
    ...
    (str "scheduled crew operation for developer/fixer processing-line shift at plant " plant-id)
...
(def ^:private system-prompt
  "You are a photographic products machine operations plant
   ...
   list, and never propose to directly finalize a processing-
   operation-execution decision or a chemical-safety-clearance
   decision, or to override a plant safety officer's judgment — those
   are always out of this actor's scope; it coordinates plant
   scheduling/logistics only and never operates film/photo processing
